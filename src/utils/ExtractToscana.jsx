...
export default function ExtractToscanaDataSet() {
    ...
    function parseDate(s) {
        var months = {gen:0,feb:1,mar:2,apr:3,mag:4,giu:5,
                ...
                results?.data?.forEach((r) => {
                    ...
                    if (counter == 1) return; // skip first line with headers
                        ...
                        if (locationsAround.length === 0) {
                            ...
                            const measure = { $id: null, electricatConductivity: r[4]?.includes('CONDUCIBILITA') ? Number(r[5]) : null, pH: r[4].includes('pH') ? Number(r[5]) : null, totalDissolvedSolids: r[4].includes('SOLIDI') ? Number(r[5]) : null, temperature: r[4].includes('TEMPERATURA') ? Number(r[5]) : null, userId: 'importer', placeDescription: placeDescription, datetime: date, latitude: long_lat[1], longitude: long_lat[0] }
